...
function add_uber_media_flickr( $sources ) {
	$sources['flickr'] = array(	'source' => 'flickr',
								'core' => true );
	return $sources;
}
...
function flickr_mmp_settings($wpsf_ubermedia_settings) {
	...
	$fields[] = 	array(
					            'id' => 'flickr-license',
					            'title' => __( 'Flickr License Options','uber-media' ),
					            'desc' => __( 'Select the license types of images returned from Flickr','uber-media' ),
					            'type' => 'checkboxes',
					            'choices' => $choices,
					            'std' => ''
					        );
...
class media_manager_plus_source_flickr extends media_manager_plus_source {
	...
	private $request_token_url = 'https://www.flickr.com/services/oauth/request_token';
 	...
 	private $settings = array	( 	'getTaggedImages' => array( 'name' => 'Tagged Images',
 									...
 									'getRecent' => array( 	'name' => 'Recent Images (All Licenses)',
 															'param' => false	),	
 								);
	...
	function getLicenses() {
		$params = array( 	'method' => 'flickr.photos.licenses.getInfo',

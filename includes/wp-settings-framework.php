...
if( !class_exists('ubermediaWordPressSettingsFramework') ){
    ...
    class ubermediaWordPressSettingsFramework {
    	...
    	/**
         * Displays the "section_description" if speicified in $wpsf_ubermedia_settings
         *
    	...
    	/**
         * Processes $wpsf_ubermedia_settings and adds the sections and fields via the WordPress settings API
         */
    	...
    	/**
         * Usort callback. Sorts $wpsf_ubermedia_settings by "section_order"
         * 
    	...
    	function generate_setting( $args )
    	{
    		...
    		switch( $type ){
    		    ...
    		    case 'checkbox':
    		        ...
    		        break;
    		    case 'checkboxes':
    		        $val = (isset($val) && $val) ? $val : array();
    		        foreach($choices as $ckey=>$cval){
    		            $select = (in_array($ckey, $val)) ? ' checked="checked"' : '';
    		            echo '<label><input type="checkbox" name="'. $this->option_group .'_settings['. $el_id .'][]" id="'. $el_id .'_'. $ckey .'" value="'. $ckey .'" class="'. $class .'"'. $select .' /> '. $cval .'</label><br />';
    		        }
    		        if($desc)  echo '<p class="description">'. $desc .'</p>';
    		        break;
    		    case 'color':
        ...
        function settings()
        {
            ...
            <form action="options.php" method="post">
        		...
        		<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save All Changes','uber-media' ); ?>" /></p>
			</form>
    		<?php
